...
class NameNodeRpcServer implements NamenodeProtocols {
  ...
  /** The RPC server that listens to requests from DataNodes */
    /**
     * 我们就可以看一下，RPC是属于hadoop-common自己弄得一个rpc server的服务
     * 在hadoop各个项目中，大家都可以用这个rpc server
     * 看过rpc server初始化的源码，大家大概都可以猜测到他这里是怎么回事，rpc server，监听一个端口号
     * 然后的话呢可以给一个rpc server绑定一大堆的小service，每个小service内部都提供了一大堆的接口，如果rpc server
     * 接收到了请求的话，就会转发给内部的小service来今星期处理
     *
     * 你的hadoop生态中的任何一个组件，如果你要启动一个http server监听某个端口号，接收某些请求，没关系
     * 底层的架构都被HttpServer2给做好了，你只要初始化一下，配置一下，加入一些映射请求的servlet
     * 你的namenode进程里就有一个组件可以监听那个端口发送过来的http前请求了
     *
     * rpc，hadoop-common给你搞好了一个RPC.Server
     * 你只要初始化一个RPC.server，然后配置一下，然后在里面加入一大堆的小service，每个service都提供一大堆的接口方法
     * 如果人家调用到了这个接口之后，RPC.Server就会转过来调用你的小service的接口方法
     *
     */
  ...
  public NameNodeRpcServer(Configuration conf, NameNode nn)
    ...
    // 这个东西里面看起来是一大堆的接口
    // 包括管理block、创建目录、常见文件、设置权限，一大堆的接口
    // ClientNamenode*****
    // 这个名字代表了什么呢？这个里面的都是client和namenode之间进行通信，需要调用的接口
    ...
    // Datanode****，代表了datanode和namenode之间进行通信，需要调用的接口
    ...
    // 这个里面的接口，转换为active namenode，转换为standby namenode，当前是否存活
    ...
    // 这个serviceRpcAddr一看就是从配置文件里解析出来的，serviceRpcServer要监听的端口号
  ...
  void start() {
    ...
    if (serviceRpcServer != null) {
      // 按照我们之前看到的那个源码，当时我们发现某个参数默认是空的，所以按照源码流程而言
      // 确实是有可能serviceRpcServer是不会启动的，因为很可能就是null
